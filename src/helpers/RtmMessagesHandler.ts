...
type Response = {
	type: 'AddMe' | 'AddMeBack' | 'MuteMyMic' | 'MuteMyVideo' | 'UnMuteMyMic' | 'UnMuteMyVideo' | 'Left';
	uid: string;
	username: string;
};

export function setMessageHandlers(rtmClient: RtmClient, channel: RtmChannel, UID: string, username: string, setUsers: React.Dispatch<React.SetStateAction<UserProps[]>>) {
	const handlerMemberJoin = (memberID: string) => memberJoined(memberID, rtmClient, UID, username);
	const handlerMessages = (message: RtmMessage, memberID: string) => messageHandler(message, memberID, UID, username, rtmClient, setUsers);
...
function messageHandler(message: RtmMessage, memberID: string, uid: string, username: string, rtmClient: RtmClient, setUsers: React.Dispatch<React.SetStateAction<UserProps[]>>) {
